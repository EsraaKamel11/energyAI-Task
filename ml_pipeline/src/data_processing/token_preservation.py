...
class TokenPreservation:
    ...
    def __init__(
        self, tokenizer: PreTrainedTokenizer, domain: str = "electric_vehicles"
    ):
    ...
    def _setup_token_preservation(self):
        ...
        logger.info(
            f"Token preservation: {preserved_count}/{total_count} terms preserved ({preserved_count/total_count*100:.1f}%)"
        )
    ...
    def tokenize_with_preservation(self, text: str, **kwargs) -> Dict[str, Any]:
        ...
        # Tokenize
        tokens = self.tokenizer(protected_text, **kwargs)
    ...
    def _restore_special_terms(
        self, tokens: Dict[str, Any], original_text: str
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "token_type_ids": tokens.get("token_type_ids", [0] * len(restored_ids))[
                : len(restored_ids)
            ],
        }
    ...
    def get_preservation_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "preservation_rate": (
                len(self.preserved_tokens) / len(self.term_mappings)
        ...
        for category, terms in self.special_terms.items():
            category_preserved = sum(
                1 for term in terms if term in self.preserved_tokens
            )
...
def create_ev_tokenizer(
    base_model: str = "microsoft/DialoGPT-medium",
) -> Tuple[PreTrainedTokenizer, TokenPreservation]:
...
def tokenize_ev_documents(
    documents: List[Dict[str, Any]],
    tokenizer: PreTrainedTokenizer,
    preservation: TokenPreservation,
) -> List[Dict[str, Any]]:
...
def analyze_token_preservation(
    documents: List[Dict[str, Any]], preservation: TokenPreservation
) -> Dict[str, Any]:
    ...
    for doc in documents:
        ...
        for term_info in verification["terms_found"]:
            ...
            if verification["preservation_status"][term_info["term"]]["preserved"]:
                analysis["category_analysis"][category]["preserved"] += term_info[
                    "count"
                ]
        ...
        doc_analysis = {
            ...
            "terms_preserved": sum(
                1
                for status in verification["preservation_status"].values()
                if status["preserved"]
            ),
            "preservation_score": verification["overall_score"],
        }
    ...
    if analysis["total_terms_found"] > 0:
        analysis["overall_preservation_rate"] = (
            analysis["total_terms_preserved"] / analysis["total_terms_found"]
        )
