...
class DataCleaner:
    def __init__(
        self, model_name: str = "all-MiniLM-L6-v2", similarity_threshold: float = 0.95
    ):
    ...
    def process(
        ...
        filter_sentences: bool = False,
        min_length: int = 20,
    ) -> pd.DataFrame:
        df[text_column] = (
            df[text_column]
            .astype(str)
            .map(lambda t: self.clean_text(t, remove_boilerplate=remove_boilerplate))
        )
        if filter_sentences:
            df[text_column] = df[text_column].map(
                lambda t: self.filter_sentences(t, min_length=min_length)
            )
