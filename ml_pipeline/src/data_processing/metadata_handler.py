...
class MetadataHandler:
    ...
    def _extract_source_metadata(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if doc_type == "pdf" or source.endswith(".pdf"):
            return {
                ...
                "extraction_method": doc.get("metadata", {}).get(
            ...
            return {
                "source_type": "web",
                "url": source,
                "scraped_date": doc.get("metadata", {}).get(
                    "timestamp", datetime.now().isoformat()
                ),
                "status_code": doc.get("metadata", {}).get("status_code", 200),
            }
        else:
            # Ensure we don't create empty structs for Parquet serialization
            original_metadata = doc.get("metadata", {})
            if not original_metadata:
                original_metadata = {"source": source, "type": "unknown"}
    ...
    def propagate_metadata_to_qa(
        self, qa_pairs: List[Dict[str, Any]], source_docs: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for qa in qa_pairs:
            ...
            if context_hash in source_lookup:
                ...
                qa["source_metadata"] = self._find_best_source_match(
                    context, source_lookup
                )
    ...
    def _find_best_source_match(
        self, context: str, source_lookup: Dict[str, Dict]
    ) -> Dict[str, Any]:
        ...
        for text_hash, metadata in source_lookup.items():
            ...
            if len(context) > 50 and any(
                keyword in context.lower() for keyword in ["ev", "charging", "electric"]
            ):
                return metadata
        return {"source_type": "unknown", "confidence": "low"}

    def add_attribution_to_qa(
