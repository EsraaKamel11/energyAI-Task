...
class QAGenerator:
    def __init__(self, config: Optional[QAGenerationConfig] = None):
        ...
        if not OPENAI_AVAILABLE:
            self.logger.warning("OpenAI module not available. Install with: pip install openai")
            self.client = None
            self.splitter = None
        else:
        ...
        if not LANGCHAIN_AVAILABLE:
            self.logger.warning("LangChain module not available. Install with: pip install langchain-text-splitters")
            self.splitter = None
        else:
    ...
    def generate_qa_pairs(self, documents: List[Dict[str, Any]], domain: str, 
        ...
        if not OPENAI_AVAILABLE or not LANGCHAIN_AVAILABLE:
            self.logger.warning("Required dependencies not available. Returning empty QA pairs.")
            return []
        
        if not self.client or not self.splitter:
            self.logger.warning("QA generator not properly initialized. Returning empty QA pairs.")
            return []
