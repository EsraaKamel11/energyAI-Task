...
class StorageManager:
    ...
    def load_from_sqlite(self, table_name: str = "data") -> pd.DataFrame:
        self.logger.info(
            f"Loading data from SQLite: {self.db_path}, table: {table_name}"
        )
        with sqlite3.connect(self.db_path) as conn:
    ...
    def save_to_parquet(self, df: pd.DataFrame, path: str) -> None:
        ...
        # Clean up any empty structs that might cause Parquet serialization issues
        df_clean = df.copy()
        for col in df_clean.columns:
            if df_clean[col].dtype == "object":
                # Replace empty dictionaries with None to avoid struct issues
                df_clean[col] = df_clean[col].apply(
                    lambda x: None if isinstance(x, dict) and not x else x
                )
